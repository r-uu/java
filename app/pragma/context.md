...
Die [Übergeordnete Kontext-Datei](https://github.com/r-uu/lib-java/blob/main/context.md) muss immer zuerst gelesen und berücksichtigt werden. Die Inhalte in der übergeordneten Datei werden durch den Inhalt dieser Datei ggf. überschrieben.
...
`r-uu.lib.java.bom` (aus lib-java) ← `r-uu.app.java.pragma.bom` ← Root `r-uu.app.java.pragma` ← Submodule.

Das App-BOM erbt vom lib-BOM (Build-/Plugin-/Property-Config, externe Dependency-Versionen)
und legt zusätzlich fest:
- App-Modul-Versionen (`r-uu.app.java.pragma.*`) via `${project.version}` — folgen bewusst der App-Version.
- **Genutzte `r-uu.lib.java.*`-Module: direkt mit Klartext-lib-Version gepinnt** — entkoppelt die
  App-Version von der lib-Version (Variante B).

**Regel für AI-Agenten:** Wenn ein Modul ein *neues* `r-uu.lib.java.*`-Artefakt nutzt, muss dafür ein
